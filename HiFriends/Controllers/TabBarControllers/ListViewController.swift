...
class ListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = .white //.mainColor()
        setupCollectionView()
        setupSearchBar()
        setupDataSource()
        reloadData()
...
extension ListViewController {
    ...
    private func configure<T: ConfiguringCell>(cellType: T.Type, with value: HiChat, for indexPath: IndexPath) -> T {
    ...
    private func setupDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Section, HiChat>(collectionView: collectionView, cellProvider: { (collectionView, indexPath, chat) -> UICollectionViewCell? in
            ...
            case .activeChats:
                return self.configure(cellType: ActiveChatViewCell.self, with: chat, for: indexPath)
            case .waitingChats:
                return self.configure(cellType: WaitingChatViewCell.self, with: chat, for: indexPath)
            }
        })
